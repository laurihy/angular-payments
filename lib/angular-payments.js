...
.factory('_Format',['Cards', 'Common', '$filter', function(Cards, Common, $filter){
  ...
  _ridiculousGetWhatTheUserTypedFunctionFML= function (e) {
    var target = e.currentTarget;
    var currentCursorPos = target.selectionStart; // grab the cursor position
    var val = angular.element(target).val(); // grab the input value
    var firstPrevious = val.charAt(currentCursorPos - 1); // grab the character that was just entered
    return firstPrevious
  }

  _ridiculousRemoveWhatTheUserTypedFunctionFML= function (e) {
    var target = e.currentTarget;
    var currentCursorPos = target.selectionStart; // grab the cursor position
    var $target = angular.element(target)
    var val = $target.val()
    $target.val(val.slice(0, currentCursorPos - 1) + val.slice(currentCursorPos)); // take out the char they just typed
  }
  ...
  var _formatCardNumber = function(e) {
      ...
      digit = _ridiculousGetWhatTheUserTypedFunctionFML(e)
  ...
  var _restrictCardNumber = function(e) {
    ...
    if(!/^\d+$/.test(digit)) {
        _ridiculousRemoveWhatTheUserTypedFunctionFML(e)
        return;
      }
  ...
  _formatCVC = function(e){
    ...
    if ((value).length > 4 && ! _hasTextSelected($target)) {
      _ridiculousRemoveWhatTheUserTypedFunctionFML(e)
      e.preventDefault()
      return
    }
  ...
  _formatForwardSlash = function(e) {
    ...
    slash =_ridiculousGetWhatTheUserTypedFunctionFML(e)
